...
| 旧编号 | 新编号 | 说明 |
|---|---|---|
| P0-1 会话持久化（→ Redis） | **P0-1 业务数据持久化（→ MySQL）** | 目标存储变更，**返工**；保留部分与变更说明并入原迭代文档 §7.1，返工交付见 §7.2（tag `v2.0.0-p0.1b`） |
...
- P0-1 的返工**并入** `docs/iterations/v2.0.0-p0.1-redis-session-store.md` 的 §7（新增 §7.2），
  **不新开迭代文档**；`docs/CHANGELOG.md` 的「二、版本流水」也不新增计划书任务以外的编号；
- 但**返工要在 git tag 上加字母后缀**：本次交付打 `v2.0.0-p0.1b`，
  `.env` 的 `PROJECT_VERSION` 同步为 `2.0.0-p0.1b`（老 tag `v2.0.0-p0.1` 永不重打）。
  > ⚠️ 与旧措辞的差异：原文写「不新开 `p0.1b`」。那指的是**不开新的迭代文档、不新增计划书任务编号**；
  > 而 tag 规则在 2026-09-23 二次修订后明确「返工用字母后缀」。
  > 两句话合起来就是现在的做法：**一份文档、一个新 tag**。
...
| P1-5 Docker 化（本地全栈 + 服务器预演） | ✅ **已完成**（5a 中间件 + 5b 应用，2026-09-24 全栈联调通过） | `2.0.0-p1.5b` | `iterations/v2.0.0-p1.5a-middleware-compose.md`<br>`iterations/v2.0.0-p1.5b-app-containers.md` |
| P1-6 模型改为全部走硅基流动（不做本地部署） | ✅ **已完成** | `2.0.0-p1.6` | `iterations/v2.0.0-p1.6-remote-models.md` |
...
> **P1-5 完成（5a + 5b，2026-09-24 全栈联调通过）**；
> **P1-6 完成**（嵌入 / 重排 / 对话 / 意图走硅基流动，不在本机加载模型）；
> **`2.0.0-p1.6b`** 是计划格之外的使用修正（会话不再因闲置消失、相同问题精确缓存），不计入 7/10。
> **`2.0.0-p1.6c`** 同样不占计划格（流式回答可停止，上滑不再被拽回底部）。其余未开始。
> 完成 **7 / 10**。
> 🎉 **P0 阶段已完成**，阶段 tag `v2.0.0-p0.4` 已打（与交付 tag `v2.0.0-p0.4b` 同一个 commit）。
> ✅ **P1-5 的阶段 tag `v2.0.0-p1.5` 已在联调通过后补打**（联调修复三处，见 p1.5b 文档 §7）。
> **下一步 = P1-7 TLS**（等备案）。
>
...
### P0-1 业务数据持久化：Redis → **MySQL** ✅ 已完成

> **状态：✅ 已完成**（tag `v2.0.0-p0.1b`，2026-09-23）。第一轮交付（`2.0.0-p0.1`）把会话落到了
> Redis，方案已作废。交付记录见 `iterations/v2.0.0-p0.1-redis-session-store.md`
> **§7.1（方案变更）与 §7.2（返工交付）**。
>
> 已交付并**保留**的资产：`SessionStore` 抽象层、契约测试体系、`RedisMemoryMonitor`。
> 已交付**退役**的部分：`RedisSessionStore` 作为会话真相源（代码暂留，供 module6 / module7 做 Redis 语义验证）。
>
> **返工实际交付**：`core/db.py`（连接层）、`core/schema.py`（查询侧表定义）、
> `core/mysql_store.py`（`MySQLSessionStore`）、`alembic/`（迁移 + `0001` 建五张表）、
> `tests/store_contract.py`（三后端共享契约）、`tests/test_module8_mysql_session_store.py`（62 项）。
> **验收 5 条全部达成**，其中第 1 条（业务侧零改动）由 `git diff --stat` 证明。
...
- **TTL 语义（`2.0.0-p1.6b` 起改为不再隐藏）**：`last_active_at` 仍在打开会话或新提问时更新，用来回答「多久没再动」。`load` / `list_ids` 不再因超期返回空，`purge_expired()` 不再归档。用户删除才删行。原先「超 TTL 后 load 返回 None」的验收第 5 条作废。
...
5. ~~会话闲置超 TTL 后 `load` 返回 None~~ → **`2.0.0-p1.6b` 改为：闲置后列表和历史仍在。**
...
> **两个可以各自独立验收**的交付，混在一起改，任何一处出问题都要同时怀疑两边。
> 拆开之后，后端接口的形状先被 module9 第 9 组锁死，前端改造就有一个**稳定的靶子**。
> **代价**：`p0.3a` 之后到 `p0.3b` 之前，知识库管理页是**坏的**
> （上传不再返回 `chunks_added`，旧前端读不到这个字段）——这是刻意选择的短期断点。
>
> - **3a 做了什么**：`core/document_repo.py`（含原子抢任务）、`core/parsing.py`（唯一解析入口）、
>   `core/queue.py`（入队抽象）、`worker/`、迁移 `0002`（`parse_started_at`）、
>   `core/vector_store.py` 的 `doc_id` 维度（**P0-4 的前置，提前做是因为 metadata 补不回来**）、
>   `api/routes/documents.py` 重写、`GET /api/v1/system/queue` 新增、
>   module9（166 项）+ `tests/acceptance_p0_3a.py`（计划书 5 条标准逐条实测，全绿）。
...
> **状态：✅ P0-4 已完成。**
> 后端（`p0.4a`：chunk_id 契约 + 引用反查 + 孤儿治理 + 重建脚本）
> + 前端（`p0.4b`：引用可点击 + 展开切片全文），两段各自独立验收。
...
1. 删除文档后，磁盘 / Chroma / MySQL 三处均无残留；  ← ✅（后端，见 `p0.4a` 文档 §5.1）
2. 历史回答点击引用 → 能通过 `chunk_id` 拿到切片正文与源文档信息；  ← ✅（后端接口；**「点击」这个交互归 `p0.4b`**）
3. 重建后同一问题检索到的内容与重建前一致（同问题同答案抽检）；  ← ✅（正文逐字一致，引用键按多重集一致）
4. Chroma 里不存在缺 `doc_id` 的切片。  ← ✅（缺 `doc_id` 0 条、孤儿 0 条）
...
> **状态：✅ 已完成（5a + 5b，2026-09-24 全栈联调通过）。**
...
> **应用代码一行未改**；联调抓到并修复 3 个真问题（网关生产配置缺口、compose 插值
> 吃掉 bcrypt 哈希、lock 文件在 Linux 下补拉整套 CUDA），修复后六容器协同、登录、
> 同步/流式问答、上传解析、引用反查、删除闭环全部实测通过。
...
> **状态：✅ 已完成（`2.0.0-p1.6`）。**
> 原方案是服务器放真实 `models/` 目录再 bind mount。按后续决定改为不在本机加载模型：
> 嵌入 `BAAI/bge-large-zh-v1.5`、重排 `BAAI/bge-reranker-v2-m3`、对话与意图都走硅基流动。
> 维度从 512 变为 1024，已重灌。回归测试仍强制走本地权重。
> 详见 `iterations/v2.0.0-p1.6-remote-models.md`。
...
- **P3-11 问答字符串缓存**：精确匹配的一小块已在 `2.0.0-p1.6b` 落地（Redis、最近使用、上限 100、知识库变更清空）。按项目隔离、近义句命中仍不做。
...
6. 服务器上同一套 compose 起得来、流式正常（P1-5；模型走硅基流动，不在服务器放权重）；
