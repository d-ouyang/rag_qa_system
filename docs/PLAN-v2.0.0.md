...
> **P1-5 完成（5a + 5b，2026-09-24 全栈联调通过）**；其余未开始。
> 完成 **6 / 10**。
> 🎉 **P0 阶段已完成**，阶段 tag `v2.0.0-p0.4` 已打（与交付 tag `v2.0.0-p0.4b` 同一个 commit）。
> ✅ **P1-5 的阶段 tag `v2.0.0-p1.5` 已在联调通过后补打**（联调修复三处，见 p1.5b 文档 §7）。
> **下一步 = P1-6 模型目录**（本机 `models` 软链联调中已验证容器内可跟随，上服务器前换真实目录），之后 P1-7。
> `2.0.0-p1.5c` 是联调后的使用优化（复制提示、Token 浮层、Chroma server、批量上传），不占计划书任务格。
>
