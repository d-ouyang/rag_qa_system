...
| P0-4 向量元数据对齐 + 知识库重建 | 🔄 **后端已完成（`p0.4a`），前端 `p0.4b` 未做** | `2.0.0-p0.4a` | `iterations/v2.0.0-p0.4a-chunk-refs.md`（后端） |
...
>
> 大版本合计：**P0-1 完成；P0-2 完成；P0-3 完成（3a 后端 + 3b 前端）；P0-4 后端完成（`p0.4a`）、
> 前端 `p0.4b` 未做；P1-5a 完成（P1-5b 未做）；其余未开始。**
> P0 阶段 tag `v2.0.0-p0.4` 要等 P0-3 + P0-4 做完 —— 现在只差 P0-4 的**前端**那一半。
>
...
> **状态：后端（chunk_id 契约 + 引用反查 + 孤儿治理 + 重建脚本）✅ 已完成，tag `v2.0.0-p0.4a`；
> 前端「点引用展开正文」⬜ 未开始，是 `p0.4b` 的全部内容。**
> 后端交接物已备好：问答接口与会话历史的 `sources[]` 已带 `chunk_id`，反查接口已上线，
> 前端只需渲染 + 调用，不需要再动后端。
>
